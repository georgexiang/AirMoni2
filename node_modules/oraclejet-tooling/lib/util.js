...
util.exec = (command) => {
  ...
  return new Promise((resolve, reject) => {
    childProcess.exec(command, {maxBuffer: 1024 * 500}, (error, stdout) => {
      if (error) {
        reject(error);
      } else {
        console.log(stdout);
...
{
  let token = CONSTANTS.APP_SRC_HYBRID_DIRECTORY;
  let index = filePath.indexOf(token);  
  if (index < 0)
  {
    token = CONSTANTS.APP_SRC_WEB_DIRECTORY;
    index = filePath.indexOf(token);
    if (index < 0)
    {
      token = CONSTANTS.APP_SRC_DIRECTORY;
      index = filePath.indexOf(token);
      if (index < 0)
      {
        index = 0;
